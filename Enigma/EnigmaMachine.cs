...
public class EnigmaMachine
{
    ...
    public void SetRotorPositions(int[] rotorPositions)
    {
        if (rotorPositions.Length != _rotors.Count)
        {
            throw new ArgumentException(
                $"Expected {_rotors.Count} rotor positions. Only received {rotorPositions.Length}",
                nameof(rotorPositions));
        }
    ...
    private void ValidateRotorPositions(int[] rotorPositions)
    {
        ...
        {
            var rotorSizesString = string.Join(", ", _rotors.Select(rotor => rotor.DiscSize - 1));

            // Not using overload with parameter name as this is being show to the user.
#pragma warning disable MA0015
            throw new ArgumentException(
                $"Rotor position(s) {string.Join(", ", rotorsOutOfBounds)} too large. Must be smaller than the size of the encryption disc ({rotorSizesString}, respectively).");
